...
for ($i=0; $row=sql_fetch_array($result); $i++) {
	...
	$item[$i]['it_rental_price'] = $row["it_rental_price"];
...
if ($i == 0) {
	...
	// 배송비 계산
	$send_cost = get_sendcost2($s_cart_id);
}
  ...
  function order_submitCheckBox_hide(){
	...
	function forderform_check(f) {
		...
		if ($('#typereceipt2').is(':checked')) {
			// 개인소득공제
			if ($('#cuse0').is(':checked')) {
				if (!$('input[name="p_typereceipt_btel"]').val() || !$('input[name="p_typereceipt_email"]').val()) {
					alert('매출증빙 휴대폰번호 또는 이메일주소를 확인하세요.');
					return;
				}
			}
			// 사업자 지출 증빙
			if ($('#cuse1').is(':checked')) {
				if (!$('input[name="p_typereceipt_bnum"]').val() || !$('input[name="p_typereceipt_email"]').val()) {
					alert('매출증빙 사업자번호 또는 이메일주소를 확인하세요.');
					return;
				}
			}
		}
