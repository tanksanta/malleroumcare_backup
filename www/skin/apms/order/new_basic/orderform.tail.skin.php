...
$(function() {
    ...
    $('#od_delivery_type').on("change", function() {
        var zip = $("#od_b_zip").val().replace(/[^0-9]/g, "");
...
function calculate_order_price() {
    ...
    var od_delivery_type_name = $("select[name=od_delivery_type] option:checked").text();
    if (od_delivery_type !== 'delivery1') {
        if ( send_cost > 0 ) {
            save_send_cost = send_cost;
        }
        if ( send_cost2 > 0 ) {
            save_send_cost2 = send_cost2;
        }
        send_cost = 0;
        send_cost2 = 0;
        ...
        $('.delivery_cost_display').hide();

    }else{
        if ( save_send_cost ) {
            send_cost = save_send_cost;
            $("input[name=od_send_cost]").val(send_cost);
        }
        if ( save_send_cost2 ) {
            send_cost2 = save_send_cost2;
    ...
    $(".od_tot_price .print_price").text(number_format(String(tot_price)) + ' 원');
    <?php if($temp_point > 0 && $is_member) { ?>
    calculate_temp_point();
    <?php } ?>
}
...
// 수급자 정보와 동일합니다.
function gumae3baesong() {
    var f = document.forderform;

    f.od_b_name.value = f.penNm.value;
    f.od_b_tel.value  = f.penConPnum.value;
