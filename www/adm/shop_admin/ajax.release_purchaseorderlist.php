...
if( !in_array($sel_field, array('od_id', 'mb_id', 'od_name', 'od_tel', 'od_hp', 'od_b_name', 'od_b_tel', 'od_b_hp', 'od_deposit_name', 'od_invoice', 'ct_delivery_num', 'ct_warehouse')) ){   //검색할 필드 대상이 아니면 값을 제거
  $sel_field = '';
}
...
//// 최고관리자가 아닐때
//if ( $ct_status == '발주대기' && $is_admin != 'super' ) {
//  $where[] = " od_writer = '{$member['mb_id']}' ";
//}

// 공급업체 검색
if($ct_supply_partner) {
  $where[] = " ct_supply_partner = '{$ct_supply_partner}' ";
}
...
foreach(array_reverse($purchase_order_steps) as $order_step) {
  if (!$order_step['deliverylist']) continue;
...
if ( $where2 || $where ) {
  if ( $is_admin != 'super' ) {
    $where2[] = " if(`ct_status` = '발주대기', `od_writer`, '{$member['mb_id']}') = '{$member['mb_id']}' ";
  }
  if ( $where2 ) {
...
foreach($orderlist as $order) {
  ...
  if ($result_ct['ct_status'] == "보유재고등록" || $result_ct['ct_status'] == "재고소진") {
    $ct_price = $result_ct['ct_status'];
  }
  ...
  if ( $now_step != $order['ct_status'] ) {
    ...
    $sql = " select count(od_id) as cnt, sum(od_cart_price) as od_cart_price, sum(od_send_cost) as od_send_cost, sum(od_send_cost2) as od_send_cost2, sum(od_cart_discount) as od_cart_discount, sum(od_cart_discount2) as od_cart_discount2, sum(od_sales_discount) as od_sales_discount from purchase_order $sql_search ";
    $now_step = $order['ct_status'];
  }

  // 20210315성훈 필요데이터 작업
  // $sql_2 = "SELECT * FROM purchase_cart WHERE od_id ='{$order['od_id']}'";
  ...
  if ($ct_status['name'] == "발주대기") {
    $od_status_name = "발주<br>대기";
    $class_type1 = "type3";
  ...
  if ($ct_status['name'] == "발주완료") {
    $od_status_name = "발주<br>완료";
    $class_type1 = "type2";
  }
  if ($ct_status['name'] == "출고완료") {
    $od_status_name = "출고<br>완료";
    $class_type1 = "type4";
    $class_type2 = ($result_ct['ct_barcode_insert'] >= $result_ct['ct_qty']) ? " disable" : "";
  }
  if ($ct_status['name'] == "입고완료") {
    $od_status_name = "입고<br>완료";
    $class_type1 = "type5";
  }
  ...
  $moreInfo = sql_fetch("
    SELECT
      ( SELECT it_name FROM purchase_cart WHERE od_id = a.od_id ORDER BY it_id ASC LIMIT 0, 1 ) AS it_name,
      ( SELECT COUNT(*) FROM purchase_cart WHERE od_id = a.od_id ) AS totalCnt
