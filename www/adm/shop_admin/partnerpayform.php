...
if ($w == 'u') {
    ...
    $sql = " select * from g5_shop_partnerpay where pp_id = '$pp_id' ";
    $pp = sql_fetch($sql);
...
// pg 설정 필드 추가
if(!sql_query(" select pp_pg from g5_shop_partnerpay limit 1 ", false)) {
    sql_query(" ALTER TABLE `g5_shop_partnerpay`
                    ADD `pp_pg` varchar(255) NOT NULL DEFAULT '' AFTER `pp_price` ", true);

    // 파트너결제 PG kcp로 설정
    sql_query(" update g5_shop_partnerpay set pp_pg = 'kcp' ");
}

// 현금영수증 필드 추가
if(!sql_query(" select pp_cash from g5_shop_partnerpay limit 1 ", false)) {
    sql_query(" ALTER TABLE `g5_shop_partnerpay`
...
<form name="fpartnerpayform" action="./partnerpayformupdate.php" method="post" onsubmit="return form_check(this);">
...
<?php echo $wrp_tag_st; ?>
    ...
    <section id="anc_spp_info">
        ...
        <div class="local_desc local_desc02">
            <p>주문 관련 기본 정보입니다.</p>
        </div>
        ...
        <div class="tbl_frm01 tbl_wrap">
            ...
            -->
            <?php 
            $show_mb_id = get_text($pp['mb_id']) ? get_text($pp['mb_id']) : $_GET['mb_id'];
            ?>
            <tr>
                <th scope="row"><label for="mb_id">아이디</label></th>
                <td><input type="text" name="mb_id" value="<?php echo $show_mb_id; ?>" id="mb_id" required class="required frm_input"></td>
            </tr>
            <!--
            <tr>
            ...
            -->
            <input type="hidden" name="pp_name" value="파트너결제" id="pp_name" required class="required frm_input">
            <input type="hidden" name="pp_price" value="0" id="pp_price" required class="required frm_input" size="15">
            <input type="hidden" name="od_id" value="" id="od_id" class="frm_input" size="20">
            <tr>
    ...
    <section id="anc_spp_pay" class="cbox">
        ...
        <div class="tbl_frm01 tbl_wrap">
            ...
            <tr>
                ...
                <td>
                    <input type="text" name="pp_receipt_price" value="<?php echo $pp['pp_receipt_price'] ? $pp['pp_receipt_price'] : ''; ?>" id="pp_receipt_price" class="frm_input" size="15" <?php echo $pp['pp_receipt_price'] ? 'readonly' : ''; ?>> 원
                    <br/>
                    * 작성하면 변경할 수 없으니 신중히 작성하여 주세요.
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <?php } else { ?>
                    <a href="javascript:;" onclick="window.open('<?php echo G5_SHOP_URL; ?>/taxsave.php?tx=partnerpay&od_id=<?php echo $pp_id; ?>', 'taxsave', 'width=550,height=400,scrollbars=1,menus=0');">현금영수증 발급</a>
                <?php } ?>
                </td>
            </tr>
            <?php
            ...
            -->
            <input type="hidden" name="pp_use" value="1" id="pp_use" class="frm_input" size="20">
            </tbody>
            </table>
        </div>
    </section>
    ...
    <div class="<?php echo $btn_class; ?>">
        ...
        <?php } else { ?>
        <a href="./partnerpaylist.php?<?php echo $qstr; ?>" class="btn btn_02">목록</a>
        <?php } ?>
        <?php if($w == 'u') { ?>
        <a href="./partnerpayformupdate.php?w=d&amp;pp_id=<?php echo $pp['pp_id']; ?>" onclick="return delete_confirm(this);" class="btn btn_02">삭제</a>
        <?php } ?>
    </div>
