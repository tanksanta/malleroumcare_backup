...
<section id ="anc_scf_payment">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        $query = "show columns from `{$g5['g5_shop_cart_naverpay_table']}` like 'od_mk' ";
        $res = sql_fetch($query);
        if (empty($res)) {
            sql_query(" ALTER TABLE `{$g5['g5_shop_cart_naverpay_table']}`
                            ADD `od_mk` varchar(50) NOT NULL DEFAULT ''
                            ; ", true);
        }
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <td>
                ...
                ?>
                    <?php 
                    if ($temp_delivery_type != $type['type']) {
                        echo "<br>";
                        $temp_delivery_type = $type['type'];
                    }
                    ?>
                    <input type="checkbox" id="de_delivery_type_<?php echo $type['val']; ?>" name="de_delivery_type_<?php echo $type['val']; ?>" value="1" <?php echo 1 == $default['de_delivery_type_' . $type['val']] ? 'checked' : ''; ?> data-type="<?php echo $type['type']; ?>" <?php echo $type['required'] == true ? 'readonly checked onclick="return false;"' : ''; ?> />
                    <label for="de_delivery_type_<?php echo $type['val']; ?>"><?php echo $type['name']; ?></ㅅ></label>
                <?php } ?>
            </td>
        </tr>
        <tr>
...
<section id="anc_scf_etc">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            <th scope="row"><label for="de_item_qa_level">상품문의</label></th>
            <td>
                 <?php echo help("선택한 회원 등급 이상 회원만 상품문의 글 작성이 가능합니다.", 50); ?>
                <select name="de_item_qa_level" id="de_item_qa_level">
                    <option value="0" <?php echo get_selected($default['de_item_qa_level'], 0); ?>>비회원</option>
                    <option value="1" <?php echo get_selected($default['de_item_qa_level'], 1); ?>>1</option>
                    <option value="2" <?php echo get_selected($default['de_item_qa_level'], 2); ?>>2</option>
                    <option value="3" <?php echo get_selected($default['de_item_qa_level'], 3); ?>>3(딜러)</option>
                    <option value="4" <?php echo get_selected($default['de_item_qa_level'], 4); ?>>4(우수딜러)</option>
                    <option value="5" <?php echo get_selected($default['de_item_qa_level'], 5); ?>>5</option>
                    <option value="6" <?php echo get_selected($default['de_item_qa_level'], 6); ?>>6</option>
                    <option value="7" <?php echo get_selected($default['de_item_qa_level'], 7); ?>>7</option>
                    <option value="8" <?php echo get_selected($default['de_item_qa_level'], 8); ?>>8</option>
                    <option value="9" <?php echo get_selected($default['de_item_qa_level'], 9); ?>>9</option>
                    <option value="10" <?php echo get_selected($default['de_item_qa_level'], 10); ?>>10</option>
                </select>
            </td>
        </tr>
        <tr>
