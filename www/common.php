...
//if ( THEMA_KEY == 'partner' ) {
    if ( partner_daegi() ) {
        if ( strpos($_SERVER['REQUEST_URI'], 'shop/wait_partner') === false && strpos($_SERVER['REQUEST_URI'], 'bbs/logout') === false ) {
            $url = G5_SHOP_URL . '/wait_partner.php';
            goto_url($url);
        }
    }
//}
if(!defined('THEMA_KEY') || THEMA_KEY == '') {
    echo "도메인별 테마 관리에서 해당도메인(". $_SERVER['HTTP_HOST'] .")을 추가해주십시오.";
    exit;
}

$mb_giup = $member['mb_giup_type'] ? true : false;

// 관리 권한 확인
$is_samhwa_admin = false;
if ( $member['mb_id'] ) {
    $sql = "SELECT * FROM g5_auth WHERE mb_id = '{$member['mb_id']}'";
    $result = sql_fetch($sql);

    if ( $result['mb_id'] ) {
        $is_samhwa_admin = true;
    }
    if ( $is_admin ) {
        $is_samhwa_admin = true;
    }
...
// 삼화 파트너 회원 확인
$is_samhwa_partner = false;
if ($member['mb_type'] == 'partner' && !partner_daegi()) {
    $is_samhwa_partner = true;
}
...
// set_cookie('right_menu_area', 'on', time() + 86400*365);
