...
if ($default['de_item_qa_level'] && !$is_member ) {
	if($move) {
	    alert("회원만 작성 가능합니다.");
	} else {
	    alert_close("회원만 작성 가능합니다.");
...
if ($default['de_item_qa_level'] && $member['mb_level'] < $default['de_item_qa_level']) {
	if($move) {
	    alert('회원등급이 낮아 작성하실 수 없습니다.');
	} else {
	    alert_close('회원등급이 낮아 작성하실 수 없습니다.');
	}
	exit;
}
