...
function printOrderView(odid) {
    order_prints_pop = window.open('./pop.order.prints.php?od_id=' + odid + '|', "order_prints_pop", "width=850, height=800, resizable = no, scrollbars = no");
}
...
$(function(){
    ...
    $( document ).on( "click", '.custom-select-box-multi li, .drop_multi_main', function() {
        list_select(this);
    ...
    $( document ).on( "click", '#list_order_prints', function() {
        ...
        var list_order_prints_pop = window.open('./pop.order.prints.php?od_id=' + ret_od_id, "order_prints_pop", "width=850, height=800, resizable = no, scrollbars = yes");

    });

   // 상품 매칭 취소
   $( document ).on( "click", '#list_matching_cancel', function() {
        var matching_cancel_pop = window.open('./pop.openmarket.item.list.php', "matching_cancel_pop", "width=1080, height=900, resizable = no, scrollbars = no");        
   });
})
...
$(function(){
    ...
    function set_input_val(parent_node, key, value) {
        ...
        if ( $(parent_node).find('input[name="' + key + '"]').length ) {
            obj = $(parent_node).find('input[name="' + key + '"], select[name="' + key + '"]');
        }else{
            obj = $(parent_node).find('input[name="' + key + '[]"], select[name="' + key + '"]');
        }
        ...
        if ( $(obj).is('input') ) {
            ...
            if ( $(obj).attr("type") == 'radio' ) {
                $(parent_node).find('input:radio[name="' + key + '"]:radio[value="' + value + '"]').prop('checked', true); 
            }

            if ( $(obj).attr("type") == 'checkbox' ) {
                $(parent_node).find('input:checkbox[name="' + key + '"]:checkbox[value="' + value + '"]').prop("checked", true); /* by NAME */
                $(parent_node).find('input:checkbox[name="' + key + '[]"]:checkbox[value="' + value + '"]').prop("checked", true); /* by NAME */
            }
            ...
            if ( $(obj).attr("type") == 'button' ) {
                setTimeout(function() {
                    $(parent_node).find('input:button[value="' + value + '"]').click();
                }, 100);
            }
        }
    ...
    function open_default_setting_form() {
        var ret = '';
        $('#search_detail_table tr').each(function(index, item) {

            var title = $(item).find('th').html();
            var content = $(item).find('td');

            var content_ret = '';
            $(content).find('input, select, h2, span.linear_span').each(function(index2, item2) {

                if ( $(this).is('input') ) {
                    var temp_id = $(item2).attr("id") || $(item2).attr("name");
                    if ( $(item2).attr("type") == 'button') {
                        content_ret += '<input type="radio" name="' + $(item2).attr("name") + '" value="' + $(item2).val() + '" id="default_' + temp_id + '" />';
                        var labelText = $(item2).val();
                        content_ret += '<label for="default_' + temp_id + '">' + labelText + '</label>';
                    }else{
                        content_ret += '<input type="' + $(item2).attr("type") + '" name="' + $(item2).attr("name") + '" value="' + $(item2).val() + '" id="default_' + temp_id + '" />';
                        ...
                        if ( $(item2).attr("type") == 'radio' || $(item2).attr("type") == 'checkbox' ) {
                            var labelText = $('label[for='+  $(item2).attr("id")  +']').text();
                            // if (labelText !== '전체') {
                                content_ret += '<label for="default_' + $(item2).attr("id") + '">' + labelText + '</label>';
                            // }
                        }
                    }
                }
                if ( $(this).is('select') ) {
                    ...
                    $(item2).find('option').each(function(index3, item3) {
                        content_ret += '<option value="' + $(item3).val() + '">' + $(item3).text() + '</option>';
                    });
            ...
            $("#default_fr_date, #default_to_date").datepicker({ changeMonth: true, changeYear: true, dateFormat: "yy-mm-dd", showButtonPanel: true, yearRange: "c-99:c+99", maxDate: "+0d" });
    ...
    $('#fdefaultsettingform_submit').click(function() {
        var formdata = $.extend({}, $('#fdefaultsettingform_form').serializeObject(), { });
    ...
    $( document ).on( "click", '#select_important', function() {
        ...
        $.map($('#samhwa_order_list_table>div.table td input[type=checkbox]:checked'), function(obj, i) {
            has_selected = true;
    ...
    // 기본검색 설정 전체 버튼 추가
    $( document ).on( "click", '#fdefaultsettingform_form input[type="checkbox"]', function() {
        var val = $(this).val();
        if (!val || val === 'all' || val === 'all2') {
            $(this).nextUntil('b').each(function(index, item) {
...
function changeDeliveryInfo(od_id, x) {
    var od_delivery_company = $(x).siblings('select[name=od_delivery_company]').val();
