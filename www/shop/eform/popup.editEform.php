...
<body>
  <div id="popupWrap">
    ...
    <div class="popupFootWrap flex">
      <button id="btnSubmitEform">내용변경</button>
...
<?php   } ?>],
    ...
    $('#btnSubmitEform').click(function() { // 내용변경
      // todo: 폼 무결성 체크
      if(!confirm('계약서의 내용을 변경하시겠습니까?')) return;
      $.post('./ajax.eform.edit.php', {status: JSON.stringify(status), uuid: '<?=$eform["uuid"]?>'}, 'json')
      .done(function(data) {
        // 변경 완료
        alert('계약서 내용변경이 완료되었습니다.');
