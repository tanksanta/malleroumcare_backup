							...
							<div class="folding_box">
									<ul class="inputbox">
										<?php for($b = 0; $b< $options[$k]["ct_qty"]; $b++){ ?>
											<li>
												<input type="number" readonly maxlength="12" oninput="maxLengthCheck(this)" value="<?=$prodList[$b]["prodBarNum"]?>"class="notall frm_input frm_input_<?=$prodListCnt?> required prodBarNumItem_<?=$prodList[$prodListCnt]["penStaSeq"]?> <?=$stoIdDataList[$prodListCnt]?>" placeholder="바코드를 입력하세요." data-frm-no="<?=$prodListCnt?>" maxlength="12">
