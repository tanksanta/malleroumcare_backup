...
//$ID = "zziny0811@thkc.co.kr"; //가입 이메일;
//$API_Key = "YzVhYjI0ZTQtYWNmMi00MDNhLWFlNmItYjExMjUxNmVmMWNi"; //API Key 
...
header('Content-type: application/json');
	...
	if($arrResponse["event"]["type"] != ""){
		...
		if($arrResponse["event"]["type"] == "document_all_signed"){// 서명완료
			...
			//}
		}elseif($arrResponse["event"]["type"] == "document_rejected"){//서명거절
...
<?php }elseif($_POST["div"] == "view_doc"){// 계약서 보기
	$response = $client->request('GET', 'https://api.modusign.co.kr/documents?offset=0&limit=1&metadatas=%7B%22dc_id%22%3A%22'.strtolower($_POST["dc_id"]).'%22%7D', [
	  'headers' => [
		'accept' => 'application/json',
		'authorization' => 'Basic '.$API_Key64,
	  ],
...
}elseif($_POST["div"] == "sign_stat"){// 서명 상황
	$response = $client->request('GET', 'https://api.modusign.co.kr/documents?offset=0&limit=1&metadatas=%7B%22dc_id%22%3A%22'.strtolower($_POST["dc_id"]).'%22%7D', [
	  'headers' => [
		'accept' => 'application/json',
		'authorization' => 'Basic '.$API_Key64,
	...
	$pen_birthday_1 = $pen_Jumin_1 = $pen_Jumin_2 = $row["penBirth"];//수급자생일
	$pen_tel_1 = $row["penConNum"];//수급자전화번호
	$pen_addr_1 = $pen_addr_2 = "(".$row["penZip"].")".$row["penAddr"].' '.$row["penAddrDtl"];//수급자주소
	...
	for($i=1;$i<($count_sale+1);$i++){//판매물품-최대 15개
		...
		$it_purchase_codeN_2		.=',{"dataLabel":"it_purchase_code'.$i.'_2","value":"'.$it_code["sale"][$i].'"}'; //구매품목코드
		$it_purchase_priceN_2		.=',{"dataLabel":"it_purchase_price'.$i.'_2","value":"'.$it_price["sale"][$i].'"}'; //구매품목급여가
	...
	for($i=1;$i<($count_rant+1);$i++){//대여물품-최대 5개
		$it_rent_categoryN_1	.= ',{"dataLabel":"it_rent_category'.$i.'_1","value":"'.$ca_name["rant"][$count_rant].'"}'; //아이템대여_품목명
		$it_rent_nameN_1		.=',{"dataLabel":"it_rent_name'.$i.'_1","value":"'.$it_name["rant"][$count_rant].'"}'; //대여이름
		$it_rent_codeN_1		.=',{"dataLabel":"it_rent_code'.$i.'_1","value":"'.$it_code["rant"][$count_rant].'"}'; //대여품목코드
		$it_rent_barcodeN_1		.=',{"dataLabel":"it_rent_barcode'.$i.'_1","value":"'.$it_barcode["rant"][$count_rant].'"}'; //대여품목바코드
		$it_rent_dateN_1		.=',{"dataLabel":"it_rent_date'.$i.'_1","value":"'.$it_date["rant"][$count_rant].'"}'; //대여품목계약일
		$it_rent_priceN_1		.=',{"dataLabel":"it_rent_price'.$i.'_1","value":"'.number_format($it_price["rant"][$count_rant]).'"}'; //대여품목급여가
		$it_rent_price_penN_1	.=',{"dataLabel":"it_rent_price_pen'.$i.'_1","value":"'.number_format($it_price_pen["rant"][$count_rant]).'"}'; //대여품목본인부담금

		$it_rent_categoryN_2	.= ',{"dataLabel":"it_rent_category'.$i.'_2","value":"'.$ca_name["rant"][$count_rant].'"}'; //아이템대여_품목명
		$it_rent_nameN_2		.=',{"dataLabel":"it_rent_name'.$i.'_2","value":"'.$it_name["rant"][$count_rant].'"}'; //대여이름
		$it_rent_codeN_2		.=',{"dataLabel":"it_rent_code'.$i.'_2","value":"'.$it_code["rant"][$count_rant].'"}'; //대여품목코드
		$it_rent_priceN_2		.=',{"dataLabel":"it_rent_price'.$i.'_2","value":"'.number_format($it_price["rant"][$count_rant]).'"}'; //대여품목급여가
		$it_rent_dateN_2		.=',{"dataLabel":"it_rent_date'.$i.'_2","value":"'.$it_date["rant"][$count_rant].'"}'; //대여품목계약일
		$it_rent_sumpriceN_1	.=',{"dataLabel":"it_rent_sumprice'.$i.'_1","value":"'.number_format($it_price["rant"][$count_rant]).'"}'; //대여품목_총액1
		$it_rent_price_penN_2	.=',{"dataLabel":"it_rent_price_pen'.$i.'_2","value":"'.number_format($it_price_pen["rant"][$count_rant]).'"}'; //대여품목본인부담금
		$it_rent_price_entN_1	.=',{"dataLabel":"it_rent_price_ent'.$i.'_1","value":"'.number_format($it_price_ent["rant"][$count_rant]).'"}'; //대여품목_공단부담금1
	}
	...
	for($i=1;$i<($count_total+1);$i++){//전체물품-최대 20개
		$it_categoryN_1	.= ',{"dataLabel":"it_category'.$i.'_1","value":"'.$ca_name["all"][$i] = $row["ca_name"].'"}'; //아이템품목명1
...
$url = $pen_sign_info.'
					...
					,"metadatas":[{"key":"entId","value":"'.$_POST["mb_entId1"].'"},{"key":"dc_id","value":"'.$_POST["dc_id1"].'"}]
					,"title":"'.$_POST["title"].'"
				}
				,"templateId":"'.$templateId.'"}';
*/
	$response = $client->request('POST', 'https://api.modusign.co.kr/documents/request-with-template', [
	  'body' => '{"document":{
					"participantMappings":[
						'.$pen_sign_info.'
						,'.$contract_sign_info.'
	...
	$mo_num = $_POST["mo_num"];
	$response = $client->request('POST', 'https://api.modusign.co.kr/documents/'.$doc_id.'/forward', [
	  'body' => '{"contacts":['.$mo_nums.']}',
	  'headers' => [
		'accept' => 'application/json',
		'authorization' => 'Basic '.$API_Key64,
		'content-type' => 'application/json',
	...
	$arrResponse = json_decode($response->getBody());
	...
	$response = $client->request('PUT', 'https://api.modusign.co.kr/documents/'.$doc_id.'/metadatas', [
	  'body' => '{"metadatas":[{"key":"dc_id","value":"sign_cancel'.strtolower($_POST["dc_id"]).'"}]}',
	  'headers' => [
		'accept' => 'application/json',
		'authorization' => 'Basic '.$API_Key64,
		'content-type' => 'application/json',
	...
	$response = $client->request('POST', 'https://api.modusign.co.kr/documents/'.$doc_id.'/cancel', [
	  'body' => '{"accessibleByParticipant":false,"message":"서명요청 취소"}',
	  'headers' => [
		'accept' => 'application/json',
		'authorization' => 'Basic '.$API_Key64,
		'content-type' => 'application/json',
	...
	$response = $client->request('PUT', 'https://api.modusign.co.kr/documents/'.$doc_id.'/metadatas', [
	  'body' => '{"metadatas":[{"key":"dc_id","value":"dc_reset'.strtolower($_POST["dc_id"]).'"}]}',
	  'headers' => [
		'accept' => 'application/json',
		'authorization' => 'Basic '.$API_Key64,
		'content-type' => 'application/json',
...
}elseif($_POST["div"] == "sign_resend"){//서명 재요청
	$response = $client->request('POST', 'https://api.modusign.co.kr/documents/'.$_POST["doc_id"].'/remind-signing', [
	  'headers' => [
		'accept' => 'application/json',
		'authorization' => 'Basic '.$API_Key64,
	  ],
