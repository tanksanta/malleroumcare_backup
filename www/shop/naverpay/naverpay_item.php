...
foreach($itemIds as $it_id) {
    ...
    // $price       = get_price($it);
    $price       = samhwa_price($it, THEMA_KEY);
    ...
    if($it['ca_id4']) {
        $cat = sql_fetch(" select ca_name from {$g5['g5_shop_category_table']} where ca_id = '{$it['ca_id4']}' ");
        $ca_name2 = $cat['ca_name'];
    }
    if($it['ca_id5']) {
        $cat = sql_fetch(" select ca_name from {$g5['g5_shop_category_table']} where ca_id = '{$it['ca_id5']}' ");
        $ca_name2 = $cat['ca_name'];
    }
    if($it['ca_id6']) {
        $cat = sql_fetch(" select ca_name from {$g5['g5_shop_category_table']} where ca_id = '{$it['ca_id6']}' ");
        $ca_name2 = $cat['ca_name'];
    }
    if($it['ca_id7']) {
        $cat = sql_fetch(" select ca_name from {$g5['g5_shop_category_table']} where ca_id = '{$it['ca_id7']}' ");
        $ca_name2 = $cat['ca_name'];
    }
    if($it['ca_id8']) {
        $cat = sql_fetch(" select ca_name from {$g5['g5_shop_category_table']} where ca_id = '{$it['ca_id8']}' ");
        $ca_name2 = $cat['ca_name'];
    }
    if($it['ca_id9']) {
        $cat = sql_fetch(" select ca_name from {$g5['g5_shop_category_table']} where ca_id = '{$it['ca_id9']}' ");
        $ca_name2 = $cat['ca_name'];
    }
    if($it['ca_id10']) {
        $cat = sql_fetch(" select ca_name from {$g5['g5_shop_category_table']} where ca_id = '{$it['ca_id10']}' ");
        $ca_name2 = $cat['ca_name'];
    }
?>
<item id="<?php echo $id; ?>">
