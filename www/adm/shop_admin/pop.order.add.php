...
<script>
    ...
    $(function() {
        ...
        function add_flexdatalist(node) {
            $(node).flexdatalist({
            ...
            }).on("select:flexdatalist",function(event, obj, options){ // 주문 상품 검색 시 발생 이벤트
                ...
                // 추가 시 변경되는 수량에 맞춰 묶음할인 적용(54 lines)
                ...
                $('.pop_order_add_item_table input[name="it_id[]"]').each(function() {
                    ...
                    if($(this).val() == it_id ) {
                        this_qty = $(this_parent).find('input[name="qty[]"]').val().replace(/[\D\s\._\-]+/g, "");
                    ...
                    for(var sale_cnt = 0; sale_cnt < item_sale_obj[it_id]['it_sale_cnt'].length; sale_cnt++){
                        ...
                        if(temp ==0)
                            ...
                            $('.pop_order_add_item_table input[name="it_id[]"]').each(function() {
                                var this_parent = $(this).closest('tr');
                                if($(this).val() == it_id ) {
                                    it_price = $(parent).find('.price').val();
        ...
        }).on("select:flexdatalist",function(event, obj, options){ // 사업소 검색 시 발생 이벤트
        ...
        $(document).on("click", ".delete_cart", function () { // 삭제 클릭 시 발생하는 이벤트
            ...
            // 삭제 시 변경되는 수량에 맞춰 묶음할인 적용(54 lines)
            var io_price = $(parent).find('.it_option option:selected').data('price');
            var price = $(parent).find('.price').val();
        ...
        $(document).on("click", ".add_cart", function () { //추가 버튼 클릭 시 발생 이벤트
            var html_node = $('.add_item_html tbody');
            $('.pop_order_add_item_table tbody').append(
                $(html_node).html()
            );

            $('.pop_order_add_item_table tbody tr').each(function(index) { // 다시 작성 클릭 시 발생 이밴트
                $(this).find('.index').text(index + 1)
            })
        ...
        $(document).on("change keyup paste", ".it_option select, input[name='qty[]'], input[name='it_price[]']", function (e) {
            ...
            if ($(this).attr('name') === 'qty[]' || $(this).attr('name') === 'io_id[]') {
                ...
                $('.pop_order_add_item_table input[name="it_id[]"]').each(function() {
                    var this_parent = $(this).closest('tr');
                    if($(this_parent).find('span[class="index"]').text() == it_index ) {
                        targets.push({
