  ...
  if($row['rental_log_division'] == "1") {//소독
    ...
    if($row['ordId']) {
      ...
      $document = '<a href="'.G5_SHOP_URL.'/eform/downloadEform.php?od_id='.$od_id.'">계약서</a>';
		...
		//추가
		$sql = "SELECT a.penId,a.penNm,HEX(a.dc_id) AS UUID FROM `eform_document` AS a INNER JOIN `eform_document_item` AS b ON a.dc_id = b.dc_id WHERE b.it_barcode='".$_POST['barcode']."' AND a.dc_status='3' AND b.it_date='".$date2."'";
		$rows2 = sql_fetch($sql);
		if($rows2['UUID']) {
