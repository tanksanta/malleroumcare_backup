...
while( $order = sql_fetch_array($result) ) {
    ...
    $data[] = [
        ...
        $order['it_admin_memo'],
        $order['od_time'],
        ($order["ct_rdy_date"]=="" || $order["ct_rdy_date"]=="0000-00-00")?"-":$order["ct_rdy_date"],
...
$search_where = "바코드 입력여부-".$barcode_text.", 배송정보입력-".$delivery_text.", 급여구분-".$gubun_text.", 발주서 다운로드-".$delivery_excel_text.", 마감시간-".$it_deadline_text.", 검색어 : 파트너ID 또는 이름(".$search_partner_text."), 상품명(".$search_it_name_text."), 수령인명(".$search_b_name_text."), 배송주소(".$search_b_addr_text."), 관리자메모(".$search_b_memo_text.")";
$title = ['카트번호','주문번호','파트너','파트너ID','상품명(옵션)','상품코드','수령인','배송주소','상세주소','수량(개)','단가(원)','공급가액(원)','부가세(원)','합계금액(원)','배송요청사항','상품요청사항','관리자메모','주문일자','출고준비변경일','출고일','출하창고','마감시간'];
// 엑셀 라이브러리 설정
include_once(G5_LIB_PATH."/PHPExcel.php");
$reader = PHPExcel_IOFactory::createReader('Excel2007');
$excel = new PHPExcel();
$sheet = $excel->getActiveSheet();
$excel->setActiveSheetIndex(0)->mergeCells('A1:V1');
$excel->setActiveSheetIndex(0)->mergeCells('J3:V3');
...
// 전체 테두리 지정
$sheet -> getStyle(sprintf("A4:V%s", ($last_row+3))) -> getBorders() -> getAllBorders() -> setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
// 전체 가운데 정렬
$sheet -> getStyle(sprintf("A1:V%s", ($last_row+3))) -> getAlignment() -> setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
// 금액 우측 정렬
...
$sheet->getStyle("A4:V4")->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('cccccc');
