...
$sql = " insert {$g5['g5_shop_order_table']}
            set od_id             = '$od_id',
                ...
                od_b_name = '{$od_member['mb_name']}',
                od_b_tel = '{$od_member['mb_tel']}',
                od_b_hp = '{$od_member['mb_hp']}',
                od_b_zip1 = '{$od_member['mb_zip1']}',
                od_b_zip2 = '{$od_member['mb_zip2']}',
                od_b_addr1 = '{$od_member['mb_addr1']}',
                od_b_addr2 = '{$od_member['mb_addr2']}',
                od_b_addr3 = '{$od_member['mb_addr3']}',
...
for($i=0; $i<count($it_ids); $i++) {
  ...
  for($k=0;$k< 1;$k++) {
    ...
    if($it['it_delivery_min_cnt']) {
      ...
      //없으면 큰박스로만 진행
      $ct_delivery_cnt = $it['it_delivery_cnt'] ? ceil($ct_qty / $it['it_delivery_cnt']) : 0;
      $ct_delivery_price = $it['it_delivery_cnt'] ? ((@round($ct_qty / $it['it_delivery_cnt']) ?: 1) * $it['it_delivery_price']) : 0;
    }
