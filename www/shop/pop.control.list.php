...
<div id="pop_add_item" class="admin_popup">
    ...
    <div class="content_option">
        <form class="form-horizontal popadditemsearch" role="form" name="popadditemsearch" action="<?=$action?>" onsubmit="return formcheck(this);" method="get" autocomplete="off" style="width:fit-content; margin: auto;">
          ...
          for(var j=0; j<list_data.length; j++) {
              for (var i = 0; i < list_data[j].length; i++) {
                  for (var ind = list_data[j][i].length - 1; ind > -1; ind--) {
                      ...
                      innerHtml += "<td style=mso-number-format:'\@'>"+list_data[j][i][ind]['prodPayCode']+'-'+list_data[j][i][ind]['prodBarNum']+'</td>';
