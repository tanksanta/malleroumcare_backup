...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        if ($row['mb_id'] == '') { // 비회원일 경우는 주문자로 링크
            $href = '<a href="./samhwa_orderlist.php?sel_field=od_name&amp;search='.$row['od_name'].'">';
        } else { // 회원일 경우는 회원아이디로 링크
            $href = '<a href="./samhwa_orderlist.php?mb_info=true&sel_field=mb_id&amp;search='.$row['mb_id'].'">';
        }
    ...
    ?>
        <tr>
            <td class="td_alignc"><a href="./samhwa_orderform.php?od_id=<?php echo $row['od_id']; ?>"><?php echo $row['od_id']; ?></a></td>
            ...
            <td class="td_numsum"><?php echo number_format($cash); ?></td>
		</tr>
    <?php
