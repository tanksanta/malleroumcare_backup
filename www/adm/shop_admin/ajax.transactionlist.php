...
if( !in_array($sel_field, array('od_all', 'od_id', 'it_name', 'it_admin_memo', 'mb_id', 'od_name', 'od_tel', 'od_hp', 'od_b_name', 'od_b_tel', 'od_b_hp', 'od_deposit_name', 'ct_delivery_num','barcode','od_ent_name')) ){   //검색할 필드 대상이 아니면 값을 제거
  $sel_field = '';
}
...
if ($fr_date && $to_date) {
  $where[] = " ct_ex_date between '$fr_date 00:00:00' and '$to_date 23:59:59' ";
}
...
<div id=\"samhwa_order_list_table\">
  <div class=\"table list-table-style wide-table\" style=\"margin-top:47px;\">
    <table>
      <thead>
        <tr style=\"margin-top:50px;\">
          <th class=\"check\">선택</th>
          <th class=\"od_info\" style=\"width:15%;\">주문정보</th>
          <th class=\"od_price\">결제금액</th>
          <th class=\"od_entNm\" style=\"width:10%;\">사업소(주문자)</th>
          <th class=\"od_b_name\" style=\"width:10%;\">수령인(받는분)</th>
          <th class=\"od_time\" style=\"width:8%;\">출고완료일</th>
          <th class=\"od_time\" style=\"width:8%;\">거래명세서</th>
          <th class=\"od_sales\" style=\"width:10%;\">영업담당자</th>
          <th class=\"od_method\" style=\"width:15%;\">발송방법</th>
...
if ( !$total_count ) {
  $ret['main'] .= "
  <div class=\"samhwa_order_list_table_no_item\" style=\"margin-top:47px;\">
    <h1>자료가 없습니다.</h1>
  </div>
  ";
}
...
foreach($orderlist as $order) {
  ...
  // 거래명세서 전송일시
  $sql_tr = "select * from `g5_transaction` where `od_id` ='".$order['od_id']."' AND `ct_id` ='".$order['cart_ct_id']."' ORDER BY tr_date DESC LIMIT 1";
  $result_tr = sql_fetch($sql_tr);
  ...
  $tr_date = '미전송';
  $tr_method = '없음';
  if ($result_tr && !$complete1) { //complete1 == 미전송 내역만 보기 !complete1 == 미전송/전송 모두 보기
    $tr_date = $result_tr['tr_date'];
    if (!$result_tr['email'] && !$result_tr['fax']) {
      $tr_method = '없음';    
    }
    else {
      $tr_method = "";
      $arr = [];
      if ($result_tr['email']) {
        array_push($arr, "(E)".$result_tr['email']);
      }
      if ($result_tr['fax']) {
        array_push($arr, "(F)".$result_tr['fax']);
      }  
      $tr_method = implode(" / ", $arr);
    }
  }
  ...
  $ret['data'] .= "
    <tr class=\"tr_{$order['cart_ct_id']} {$class_c1} {$class_c2}\">
      <td align=\"center\" class=\"check\">
        <input type=\"checkbox\" class=\"send_chk\" name=\"od_id[]\" id=\"check_{$order['cart_ct_id']}\" value=\"{$order['cart_ct_id']}\" data-od-id=\"{$order['od_id']}\" accumul_mark=\"Y\">
        <label for=\"check_{$order['cart_ct_id']}\">&nbsp;</label>
      </td>
      <td align=\"left\" class=\"od_info\" style=\"width:15%;\">
      ...
      <td align=\"center\" class=\"od_entNm\" style=\"width:10%;\">
        {$mb_shorten_info}{$mb_entNm}
      </td>
      <td align=\"center\" class=\"od_b_name\" style=\"width:10%;\">
        {$order['od_b_name']}
      </td>
      <td align=\"center\" class=\"od_time\"style=\"width:8%;\">
        {$ct_ex_date}
      </td>
      <td align=\"center\" class=\"od_time\"style=\"width:8%;\">
        {$tr_date}
      </td>
      <td align=\"center\" class=\"od_sales\" style=\"width:10%;\">
        {$sale_manager}
      </td>
      <td align=\"center\" class=\"od_method\" style=\"width:15%;\">
        {$tr_method}
      </td>
      <td align=\"center\">
      <a href='#' class='downloadBtn' style=\"display: inline-block;
      width: 100px;
      height: 35px;
      line-height: 35px;
      border-radius: 5px;
      ...
      font-size: 12px;
      color: #333;
      text-align: center;\" data-id='{$order["od_id"]}'  data-ct='{$order["cart_ct_id"]}' >다운로드</a>
      </td>
    </tr>
  ";
