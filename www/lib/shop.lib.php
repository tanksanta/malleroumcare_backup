...
class item_list
{
  ...
  function run() {
    ...
    if ($this->query) {
      ...
      if ($this->ca_id || $this->ca_id2 || $this->ca_id3 || $this->ca_id4 || $this->ca_id5 || $this->ca_id6 || $this->ca_id7 || $this->ca_id8 || $this->ca_id9 || $this->ca_id10) {
        $where_ca_id = array();
        if ($this->ca_id) {
        ...
        if ($this->ca_id7) {
          $where_ca_id[] = " ca_id7 like '{$this->ca_id7}%' ";
        }
        if ($this->ca_id8) {
          $where_ca_id[] = " ca_id8 like '{$this->ca_id8}%' ";
        }
        if ($this->ca_id9) {
          $where_ca_id[] = " ca_id9 like '{$this->ca_id9}%' ";
        }
        if ($this->ca_id10) {
          $where_ca_id[] = " ca_id10 like '{$this->ca_id10}%' ";
        }
...
function get_item_options($it_id, $subject, $is_div='', $sb = '')
{
    ...
    if($subj_count > 1) {
        ...
        $select .= '<option value="">'.$show_subj.' 선택하세요</option>'.PHP_EOL;
        for($i=0; $row=sql_fetch_array($result); $i++) {
            $io_price = samhwa_opt_price($row, THEMA_KEY);
            // echo $aslang['io_price'];

            if($io_price >= 0)
                $price = '&nbsp;&nbsp;+ '.number_format($io_price).'원'; //원
            else
                $price = '&nbsp;&nbsp; '.number_format($io_price).'원'; //원
            /*
            if($row['io_price'] >= 0)
...
// 상품 추가옵션
function get_item_supply($it_id, $subject, $is_div='', $sb = '')
{
    global $g5, $aslang;
...
//비유통 제외
function get_sendcost2($cart_id, $selected=1, $update=0)
{
    global $default, $g5;
...
// 상품별 배송비
function get_item_sendcost($it_id, $price, $qty, $cart_id, $update=0)
{
    global $g5, $default;
...
function get_item_sendcost_by_default_case($price) {
    global $default;
