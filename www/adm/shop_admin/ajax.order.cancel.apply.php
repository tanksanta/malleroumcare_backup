...
$od_cancel_reason = trim($_POST['od_cancel_reason']);
$od_cancel_memo = trim($_POST['od_cancel_memo']);
...
$new_od_status = '입고대기';
if ($od_status['cancelable'] || $od_status['direct_cancel']) {
    $new_od_status = '취소';
}

// 이미 취소 관련 상태인경우 변경하지 않는다.
if ( $od_status['cancellist'] == true || $od_status['val'] == '취소' ) {
    $new_od_status = $od['od_status'];
}
...
$sql = " update {$g5['g5_shop_order_table']} set 
    ...
    od_cancel_time = now(),
    od_status = '{$new_od_status}'
    where od_id = '{$od_id}' ";

sql_query($sql);

if($od['od_tno'] && $new_od_status == '취소' ) {
    switch($od['od_pg']) {
        case 'lg':
            ...
            $pg_ret = $xpay->Response_Code() . ' / ' . $xpay->Response_Msg();
            break;
        case 'inicis':
        ...
        default:
            ...
            // locale 설정 초기화
            setlocale(LC_CTYPE, '');
            $pg_ret = $kcp_json_ret['code'] . ' / ' . $kcp_json_ret['msg'];
    }
    set_order_admin_log($od['od_id'], '관리자 PG('.$od['od_pg'].') 결제 취소 요청 : '. $pg_ret);
}

set_order_admin_log($od_id, get_text($od_cancel_reason) . ' 취소 신청');

$ret = array(
    'result' => 'success',
    'msg' => '취소신청 되었습니다. 이미 취소관련 상태면 상태를 변경하지 않습니다.',
);
