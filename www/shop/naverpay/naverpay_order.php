...
if (defined('APMS_MKT')) {
    $od_mk = APMS_MKT ? APMS_MKT : $member['mb_recommend']; // 2019-09-16 : 아미나빌더 파트너 기능 쿠키정보
}
else {
    $od_mk = $member['mb_recommend']; // 2019-09-16 : 아미나빌더 파트너 기능 쿠키정보
}
// wetoz : naverpayorder
...
for($i=0; $i<$count; $i++) {
    ...
    $sql = " INSERT INTO {$g5['g5_shop_cart_naverpay_table']}
                    ( MallManageCode, mb_id, it_id, it_name, it_sc_type, it_sc_method, it_sc_price, it_sc_minimum, it_sc_qty, ct_price, ct_point, ct_point_use, ct_stock_use, ct_option, ct_qty, ct_notax, io_id, io_type, io_price, ct_time, ct_ip, ct_send_cost, ct_select, ct_select_time, s_cart_id, od_mk )
                VALUES ";
    $ct_count = 0;
    // wetoz : naverpayorder
