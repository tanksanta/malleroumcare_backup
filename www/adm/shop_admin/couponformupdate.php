...
$mb_id_arr = explode(',', $_POST['mb_id']);
for($i = 0; $i < count($mb_id_arr); $i++) {
  $mb_id_arr[$i] = trim($mb_id_arr[$i]);
}
...
if($w == '') {
  if($_POST['chk_all_mb']) {
    ...
    $sql = " select mb_id from {$g5['member_table']} where mb_id IN ('". implode("' , '", $mb_id_arr) ."') and mb_leave_date = '' and mb_intercept_date = '' ";
    $row = sql_fetch($sql);
    if(!$row['mb_id'])
  ...
  // 우선 기존 적용대상 유저 다 지움
  $cp_no = $cp['cp_no'];
  sql_query(" delete from g5_shop_coupon_member where cp_no = '{$cp_no}' ", true);
...
if($w == '' && ($_POST['cp_sms_send'] || $_POST['cp_email_send'] || $_POST['cp_alimtalk_send'])) {
  ...
  if($_POST['chk_all_mb']) {
    ...
    $sql = " select mb_id, mb_name, mb_hp, mb_email, mb_mailling, mb_sms
                from {$g5['member_table']}
                where mb_id IN ('". implode("' , '", $mb_id_arr) ."') ";
  }
