...
//미리보기용 데이터 삭제:어제 날짜 이전  데이터 삭제
$beforeDay = date("Y-m-d", strtotime($day." -1 day"))." 00:00:00";
$sql = "DELETE FROM a, b,c
    USING eform_document AS a
	LEFT JOIN eform_document_item AS b
    ON a.dc_id = b.dc_id
	LEFT JOIN eform_document_log AS c
    ON a.dc_id = c.dc_id
    WHERE a.dc_status='10' AND a.dc_datetime < '{$beforeDay}'";
sql_query($sql);
      ...
      <tr>
        ...
        <td>
          <select name="sel_field" id="sel_field">
            ...
            <option value="penLtmNum" <?php echo get_selected($sel_field, 'penLtmNum'); ?>>수급자번호</option>
          </select>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr class="<?php echo $bg; ?>">
		...
		<td align="center" onClick="go_detail('<?=$row["uuid"]?>')"><?=$row["penTypeNm"]; ?></td>
		<td align="center" onClick="go_detail('<?=$row["uuid"]?>')"><?=$row["it_count"]; ?></td>
		<td align="right" onClick="go_detail('<?=$row["uuid"]?>')"><?=number_format($row["t_price"]); ?></td>
		<td align="center" onClick="go_detail('<?=$row["uuid"]?>')"><?=substr($row["dc_datetime"],0,16); ?></td>
		<td align="center" onClick="go_detail('<?=$row["uuid"]?>')"><?=($row["dc_sign_send_datetime"] == "0000-00-00 00:00:00" || $row["dc_sign_send_datetime"] == "")?"-":substr($row["dc_sign_send_datetime"],0,16); ?></td>
		<td align="center" onClick="go_detail('<?=$row["uuid"]?>')"><?=($row["dc_sign_datetime"] == "0000-00-00 00:00:00")?"-":substr($row["dc_sign_datetime"],0,16); ?></td>
		<td align="center" onClick="go_detail('<?=$row["uuid"]?>')"><?=$dc_status; ?></td>
...
function downloadExcel() {
    var href = './eform.excel.download.php';
...
function go_detail(dc_id){
	location.href="./eform_detail.php?dc_id="+dc_id+"&<?=str_replace('&amp;','&',$qstr1)?>&page=<?=$page?>";
}
</script>
