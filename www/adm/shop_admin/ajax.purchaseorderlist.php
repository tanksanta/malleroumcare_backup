...
$ret = array();
$ct_status_info = get_purchase_step($ct_status);
$show_ct_status = $ct_status_info['chulgo'] ? $ct_status_info['name'] . '<span>(' . $ct_status_info['chulgo'] . ')</span>' : $ct_status_info['name'];

$next_step = get_next_purchase_step($ct_status);
$prev_step = get_prev_purchase_step($ct_status);

if ( $next_step ) {
  $show_next_status = '<span class="btn large"><button id="change_next_step_for_purchase_order" data-next-step-val="'. $next_step['val'] .'">선택 '. $next_step['name'] .'단계로 변경</button></span>';
}else{
  $show_next_status = '';
}

if ( $prev_step ) {
  $show_prev_status = '<span class="btn large"><button id="change_prev_step_for_purchase_order" data-prev-step-val="'. $prev_step['val'] .'">선택 '. $prev_step['name'] .'단계로 되돌리기</button></span>';
}else{
  $show_prev_status = '';
}
...
//분류
$ret['main'] = "
  <div id=\"samhwa_order_list_table\" class='purchase_order_list'>
...
foreach($orderlist as $order) {
  ...
  if (in_array($order['ct_status'], ['보유재고등록', '재고소진'])) {
    $ct_price = $order['ct_status'];
  }
  ...
  if ( $now_step != $order['ct_status'] ) {
    $ct_status_info = get_purchase_step($order['ct_status']);
    $show_ct_status = $ct_status_info['chulgo'] ? $ct_status_info['name'] . '<span>(' . $ct_status_info['chulgo'] . ')</span>' : $ct_status_info['name'];

    $next_step = get_next_purchase_step($order['ct_status']);
    $prev_step = get_prev_purchase_step($order['ct_status']);
...
//    if ($ct_status_info['name'] == "재고소진" || $ct_status_info['name'] == "보유재고등록") {
//      $status_info = "총 {$total_result['cnt']}건";
//    } else {
//      $status_info = "총 {$total_result['cnt']}건 / 합계: ₩ {$total_result['price']}원";
//    }
    $status_info = "총 {$total_result['cnt']}건";
