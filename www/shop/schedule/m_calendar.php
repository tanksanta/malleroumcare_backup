  ...
  <div x-data="app()" x-init="[initDate(), getNoOfDays()]" x-cloak>
    <div x-data="select({ value: 'all', valueInModal: '', placeholder: '담당자' })" x-init="init()"
      ...
      <div class="flex-1 px-5 dark:bg-gray-700 bg-gray-50 overflow-y-auto" id="table">
        <div class="flex justify-between md:py-8 py-5 sticky top-0 bg-gray-50">
          <div class="flex items-center font-bold text-lg" x-text="$moment(select_date).format('DD. dd')">
          </div>
          <div class="flex">
            <div class="flex items-center font-bold text-lg"
                 x-text="'총 주문 ' + (filter_mb_id == '' ? schedules.filter(e => e.type === 'schedule').length : schedules.filter(e => e.type === 'schedule').filter(e => mb_type === 'default' ? e.od_mb_id == filter_mb_id : e.partner_manager_mb_id == filter_mb_id).length) + '건 / ' + '완료 주문 ' + (filter_mb_id == '' ? schedules.filter(e => e.type === 'schedule' && e.status === '완료').length : schedules.filter(e => e.type === 'schedule' && e.status === '완료').filter(e => mb_type === 'default' ? e.od_mb_id == filter_mb_id : e.partner_manager_mb_id == filter_mb_id).length) + '건'">
            </div>
          </div>
        </div>
        <div
        ...
        <template
          ...
          <div class="border-b py-2 border-gray-400 border-dashed item">
            ...
            <div class="flex flex-row">
              <div class="flex-1 flex items-center justify-start">
                <a tabindex="0"
                   ...
                   x-text="((item.status === '완료') ? '[설치완료]' : '[설치 예정]') + item.it_name + ' ' + item.ct_qty + '개'"
                   비></a>
              </div>
            ...
            <p class="text-sm pt-2 leading-4 leading-none text-gray-800 dark:text-gray-100"
               x-text="'연락처 : ' + (item.od_b_hp ? item.od_b_hp : '없음')"></p>
            <p class="text-sm pt-2 leading-4 leading-none text-gray-800 dark:text-gray-100"
            ...
            <p class="text-sm pt-2 leading-4 leading-none text-gray-800 dark:text-gray-100"
               :class="{'hidden': <?php if ($member["mb_level"] < 9 && $member["mb_type"] === 'default') echo 'false'; else echo 'true'; ?>}"
               x-text="'<?php if ($member["mb_level"] < 9 && $member["mb_type"] === 'default') echo '담당연락처'; else echo '업체연락처'; ?> : ' + (item.partner_manager_hp ? item.partner_manager_hp : '없음')">
            </p>
            <p class="text-sm pt-2 leading-4 leading-none text-gray-800 dark:text-gray-100"
               x-text="'배송지 : ' + item.od_b_addr1 + (item.od_b_addr2 ? item.od_b_addr2 :'')"></p>
            <p class=" text-sm pt-2 leading-4 leading-none text-gray-800 dark:text-gray-100"
               x-text="'요청사항 : ' + (item.prodMemo ? item.prodMemo : '없음')"></p>
          </div>
        </template>
      </div>
