...
// 주문상품의 상태가 주문인지 체크
$sql = " select SUM(IF(ct_status = '입금', 1, 0)) as od_count3,
...
if ($type == 1) {
    ...
    if($od['od_cancel_price'] > 0 || $ct['od_count1'] != $ct['od_count3']) {
        alert("취소할 수 있는 주문이 아닙니다.", G5_SHOP_URL."/orderinquiryview.php?od_id=$od_id&amp;uid=$uid");
    }
} else {
    alert("잘못된 요청입니다.", G5_SHOP_URL."/orderinquiryview.php?od_id=$od_id&amp;uid=$uid");
}
...
if($od['od_tno']) {
    ...
    set_order_admin_log($od['od_id'], '주문자 PG('.$od['od_pg'].') 결제 취소 요청 : '. $pg_ret);
}
