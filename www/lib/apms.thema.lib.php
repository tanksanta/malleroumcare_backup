...
if(USE_G5_THEME && defined('G5_THEME_PATH')) {
	...
	$sql = "SELECT * FROM g5_domain_thema WHERE dt_domain = '{$host}'";
	$thema_result = sql_fetch($sql);
	// echo $sql;
	// print_r2($thema_result);
	// exit;

	if ( $thema_result['dt_thema'] ) {
		$thema = $thema_result['dt_thema'];
		$thema_key = $thema_result['dt_key'];
	}else{
		$thema_key = '';
	}
